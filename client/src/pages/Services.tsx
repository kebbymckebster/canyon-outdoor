...
export default function Services() {
  return <><SiteHeader /><main id="main-content" className="bg-[#e0e7d9] pt-32 text-[#1d2b20] md:pt-40"><section className="soft-mesh relative overflow-hidden pb-24 md:pb-32"><div className="container relative"><PageIntro eyebrow="Designed for real life" title={<>MORE GREEN.<br />MORE OUTSIDE.</>} copy="We balance planting, shade, water, and durable materials around the daily rituals that make an outdoor space worth using." />
    <div className="mt-14 grid gap-4 md:grid-cols-3">{offerings.map(({ title, copy, image, icon: Icon, dark }) => <article key={title} className={`overflow-hidden rounded-[1.5rem] p-6 ${dark ? "bg-[#173221] text-white" : "plant-glass"}`}><Icon className={dark ? "text-[#b9d3b5]" : "text-[#52705b]"} /><h2 className="display mt-10 text-3xl font-semibold">{title}</h2><p className={`mt-4 text-sm leading-6 ${dark ? "text-white/70" : "text-[#1d2b20]/65"}`}>{copy}</p><img src={image} alt="" className="mt-7 aspect-[1.35/1] w-full rounded-2xl object-cover" loading="lazy" decoding="async" /></article>)}</div>
    <p className="mt-6 max-w-2xl text-xs leading-5 text-[#1d2b20]/60">Canyon Outdoor provides landscape design, outdoor living concepts, hardscape concepts, and property visualization. These are planning and visualization services only. Canyon Outdoor is not a licensed contractor or construction company. Construction work, permitting, engineering, and trade services are handled by qualified professionals when appropriate.</p></div></section></main><SiteFooter /></>;
}
